...
class LoginProvider {
  ...
  const LoginProvider({
    ...
    this.animated = true,
  }) : assert(button != null || icon != null, 'Either [button] or [icon] must be provided for the login provider.');
...
class _AnimationTimeDilationDropdown extends StatelessWidget {
  const _AnimationTimeDilationDropdown({required this.onChanged, this.initialValue = 1.0});
        ...
        children: <Widget>[
          ...
          SizedBox(
            ...
            child: CupertinoPicker(
              scrollController: FixedExtentScrollController(initialItem: animationSpeeds.indexOf(initialValue.toInt())),
              itemExtent: 30,
...
class __HeaderState extends State<_Header> {
  ...
  double getEstimatedTitleHeight() {
    ...
    final renderParagraph = RenderParagraph(
      TextSpan(
        text: widget.title,
        style: theme.textTheme.displaySmall!.copyWith(fontSize: widget.loginTheme.beforeHeroFontSize),
      ),
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    const gap = 5.0;
    final logoHeight = min((widget.height - MediaQuery.of(context).padding.top) - _titleHeight - gap, kMaxLogoHeight);
    ...
    if (widget.titleTag != null && !isNullOrEmpty(widget.title)) {
      ...
      title = Text(widget.title!, key: kTitleKey, style: theme.textTheme.displaySmall);
    } else {
      title = null;
    }
...
class _FlutterLoginState extends State<FlutterLogin> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _loadingController = AnimationController(vsync: this, duration: loadingDuration)..addStatusListener((status) {
    ...
    return theme.copyWith(
      ...
      inputDecorationTheme: theme.inputDecorationTheme.copyWith(
        ...
        fillColor:
            ...
            Color.alphaBlend(primaryOrWhite.withValues(alpha: .07), Colors.grey.withValues(alpha: .04)),
        ...
        errorBorder:
            ...
            inputTheme.border ??
            OutlineInputBorder(borderSide: BorderSide(color: errorColor), borderRadius: roundBorderRadius),
        focusedErrorBorder:
            inputTheme.focusedErrorBorder ??
            inputTheme.border ??
            OutlineInputBorder(borderSide: BorderSide(color: errorColor, width: 1.5), borderRadius: roundBorderRadius),
        disabledBorder:
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.footer != null) {
      footerWidget = Padding(
        ...
        child: Text(widget.footer!, style: theme.textTheme.titleSmall, textAlign: TextAlign.center),
      );
    }
        ...
        body: Stack(
          children: <Widget>[
            if (widget.backgroundImage == null)
              ...
              Container(
                decoration: BoxDecoration(image: DecorationImage(image: widget.backgroundImage!, fit: BoxFit.cover)),
              ),
