...
MaterialColor getMaterialColor(Color color) {
  return Colors.primaries.firstWhere(
    ...
    orElse: () => MaterialColor(color.toARGB32(), <int, Color>{for (final entry in shades.entries) entry.value: color}),
  );
}
...
Color darken(Color color, [double amount = .1]) {
  assert(amount >= 0 && amount <= 1, 'The darken amount must be between 0.0 and 1.0 (was $amount)');
...
Color lighten(Color color, [double amount = .1]) {
  assert(amount >= 0 && amount <= 1, 'The lighten amount must be between 0.0 and 1.0 (was $amount)');
