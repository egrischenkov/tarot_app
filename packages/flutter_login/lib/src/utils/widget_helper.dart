...
Flushbar<void> showSuccessToast(BuildContext context, String title, String message, [Duration? duration]) {
  return Flushbar(
    title: title,
    message: message,
    icon: const Icon(Icons.check, size: 28, color: Colors.white),
    duration: duration ?? const Duration(seconds: 4),
    backgroundGradient: LinearGradient(colors: [Colors.green[600]!, Colors.green[400]!]),
...
Flushbar<void> showErrorToast(BuildContext context, String title, String message) {
  return Flushbar(
    ...
    backgroundGradient: LinearGradient(colors: [Colors.red[600]!, Colors.red[400]!]),
    onTap: (flushbar) => flushbar.dismiss(),
  )..show(context);
}
