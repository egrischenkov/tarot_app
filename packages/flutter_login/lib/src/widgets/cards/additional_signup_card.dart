...
class _AdditionalSignUpCard extends StatefulWidget {
  _AdditionalSignUpCard({
    ...
    if (formFields.isEmpty) {
      ...
      throw RangeError('More than 6 formFields are not displayable, you provided ${formFields.length}');
    }
  }
...
class _AdditionalSignUpCardState extends State<_AdditionalSignUpCard> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _nameControllers = {
      for (final formField in widget.formFields) formField.keyName: TextEditingController(text: formField.defaultValue),
    };

    if (_nameControllers.length != widget.formFields.length) {
      throw ArgumentError('Some of the formFields have duplicated names, and this is not allowed.');
    }
  ...
  Future<bool> _submit() async {
    ...
    switch (auth.authType) {
      case AuthType.provider:
        error = await auth.onSignup!(SignupData.fromProvider(additionalSignupData: auth.additionalSignupData));
    ...
    if (!isNullOrEmpty(error)) {
      ...
      if (mounted) {
        showSuccessToast(context, messages.flushbarTitleSuccess, messages.signUpSuccess, const Duration(seconds: 4));
      }
