...
class _ConfirmRecoverCardState extends State<_ConfirmRecoverCard> with SingleTickerProviderStateMixin {
  ...
  Future<bool> _submit() async {
    ...
    final error = await auth.onConfirmRecover!(_code, LoginData(name: auth.email, password: auth.password));
    ...
    if (error != null) {
      ...
      if (mounted) {
        showSuccessToast(context, messages.flushbarTitleSuccess, messages.confirmRecoverSuccess);
      }
  ...
  Widget _buildConfirmPasswordField(double width, LoginMessages messages) {
    return AnimatedPasswordTextFormField(
      ...
      labelStyle: widget.labelStyle,
      textStyle: widget.textStyle,
      prefixIconColor: widget.suffixIconColor,
    );
  }
      ...
      child: Card(
        child: Container(
          ...
          child: Form(
            ...
            child: Column(
              children: <Widget>[
                Text(messages.confirmRecoverIntro, textAlign: TextAlign.center, style: theme.textTheme.bodyMedium),
