...
class _ConfirmSignupCardState extends State<_ConfirmSignupCard> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _fieldSubmitController = AnimationController(vsync: this, duration: const Duration(milliseconds: 1000));
  }
  ...
  Future<bool> _submit() async {
    ...
    if (mounted) {
      showSuccessToast(context, messages.flushbarTitleSuccess, messages.confirmSignupSuccess);
    }
  ...
  Future<bool> _resendCode() async {
    ...
    if (mounted) {
      showSuccessToast(context, messages.flushbarTitleSuccess, messages.resendCodeSuccess);
    }
  ...
  Widget _buildResendCode(ThemeData theme, LoginMessages messages) {
    return ScaleTransition(
      ...
      child: MaterialButton(
        ...
        child: Text(messages.resendCodeButton, style: theme.textTheme.bodyMedium, textAlign: TextAlign.left),
      ),
    );
  }
