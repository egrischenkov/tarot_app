...
class AuthCardState extends State<AuthCard> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _routeTransitionController = AnimationController(vsync: this, duration: const Duration(milliseconds: 1100));

    _cardSizeAnimation = Tween<double>(begin: 1, end: cardSizeScaleEnd).animate(
      CurvedAnimation(
        parent: _routeTransitionController,
        curve: const Interval(0, .27272727 /* ~300ms */, curve: Curves.easeInOutCirc),
      ),
    );
    ...
    ).animate(CurvedAnimation(parent: _routeTransitionController, curve: const Interval(.27272727, .5)));

    _cardOverlaySizeAndOpacityAnimation = Tween<double>(
      begin: 1,
      end: 0,
    ).animate(CurvedAnimation(parent: _routeTransitionController, curve: const Interval(.5, .72727272)));
    ...
    _cardRotationAnimation = Tween<double>(begin: 0, end: pi / 2).animate(
      CurvedAnimation(
        parent: _routeTransitionController,
        curve: const Interval(.72727272, 1 /* ~300ms */, curve: Curves.easeInOutCubic),
      ),
    );
  }
  ...
  void _changeCard(int newCardIndex) {
    ...
    setState(() {
      _pageController.animateToPage(newCardIndex, duration: const Duration(milliseconds: 500), curve: Curves.ease);
  ...
  Widget _buildLoadingAnimator({required ThemeData theme, Widget? child}) {
    ...
    overlay = Padding(
      ...
      child: AnimatedBuilder(
        ...
        child: DecoratedBox(decoration: BoxDecoration(color: theme.colorScheme.secondary)),
      ),
    );

    overlay = ScaleTransition(
      scale: _cardOverlaySizeAndOpacityAnimation,
      child: FadeTransition(opacity: _cardOverlaySizeAndOpacityAnimation, child: overlay),
    );
  ...
  Widget _changeToCard(BuildContext context, int index) {
    ...
    Future<bool> requireSignUpConfirmation() async {
      final confirmSignupRequired =
          await auth.confirmSignupRequired?.call(LoginData(name: auth.email, password: auth.password)) ?? true;
      ...
      case _loginPageIndex:
        return _buildLoadingAnimator(
          ...
          child: _LoginCard(
            ...
            labelStyle: widget.loginTheme?.labelStyle,
            textStyle: widget.loginTheme?.textFieldStyle,
            prefixIconColor: widget.loginTheme?.prefixIconColor,
            suffixIconColor: widget.loginTheme?.suffixIconColor,
          ),
        );
      case _recoveryIndex:
        return _RecoverCard(
      ...
      child: TransformerPageView(
        ...
        itemBuilder: (BuildContext context, int index) {
          if (widget.scrollable) {
            ...
            return Align(alignment: Alignment.topCenter, child: _changeToCard(context, index));
          }
        },
      ),
    );
