...
Interval _getInternalInterval(
  ...
  return Interval(start + (end - start) * externalStart, start + (end - start) * externalEnd, curve: curve);
}
...
class _AnimatedTextFormFieldState extends State<AnimatedTextFormField> {
  ...
  void initState() {
    ...
    if (loadingController != null) {
      ...
      suffixIconOpacityAnimation = Tween<double>(begin: 0, end: 1).animate(
        CurvedAnimation(parent: loadingController, curve: _getInternalInterval(.65, 1, interval.begin, interval.end)),
      );
    ...
    if (inertiaController != null) {
      fieldTranslateAnimation = Tween<double>(begin: 0, end: sign * 15.0).animate(
        CurvedAnimation(
    ...
    if (widget.userType == LoginUserType.intlPhone) {
      _phoneNumberInitialValue = PhoneNumber(isoCode: widget.initialIsoCode ?? 'US', dialCode: '+1');
  ...
  void _updateSizeAnimation() {
    ...
    sizeAnimation = Tween<double>(begin: 48, end: widget.width).animate(
      CurvedAnimation(
        parent: loadingController,
        curve: _getInternalInterval(.2, 1, interval.begin, interval.end, Curves.linearToEaseOut),
    ...
    return InputDecoration(
      ...
      suffixIcon: _buildInertiaAnimation(
        widget.loadingController != null
            ? FadeTransition(opacity: suffixIconOpacityAnimation, child: widget.suffixIcon)
            : widget.suffixIcon,
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
      ...
      inputField = Padding(
        ...
        child: InternationalPhoneNumberInput(
          ...
          selectorConfig: SelectorConfig(
            ...
            trailingSpace: false,
            countryComparator:
                (c1, c2) => int.parse(c1.dialCode!.substring(1)).compareTo(int.parse(c2.dialCode!.substring(1))),
          ),
    ...
    if (widget.loadingController != null) {
      inputField = ScaleTransition(
        ...
        child: AnimatedBuilder(
          ...
          builder:
              (context, child) =>
                  ConstrainedBox(constraints: BoxConstraints.tightFor(width: sizeAnimation.value), child: child),
          child: inputField,
        ),
      );
    }
    ...
    if (widget.inertiaController != null) {
      inputField = AnimatedBuilder(
        ...
        builder:
            (context, child) => Transform.translate(offset: Offset(fieldTranslateAnimation.value, 0), child: child),
        child: inputField,
      );
    }
...
class _AnimatedPasswordTextFormFieldState extends State<AnimatedPasswordTextFormField> {
  ...
  Widget build(BuildContext context) {
    return AnimatedTextFormField(
      ...
      prefixIcon: Icon(FontAwesomeIcons.lock, size: 20, color: widget.prefixIconColor),
      suffixIcon: GestureDetector(
        ...
        child: AnimatedCrossFade(
          ...
          layoutBuilder: (Widget topChild, _, Widget bottomChild, _) {
            return Stack(alignment: Alignment.center, children: <Widget>[bottomChild, topChild]);
          },
          firstChild: Icon(Icons.visibility, size: 25, semanticLabel: 'show password', color: widget.suffixIconColor),
          secondChild: Icon(
