...
class __HeroTextContentState extends State<_HeroTextContent> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _controller = AnimationController(vsync: this, duration: const Duration(milliseconds: 600))..addListener(() {
        ...
        return _HeroTextContent(
          ...
          viewState:
              viewState == ViewState.shrunk
                  ? (flightDirection == HeroFlightDirection.push ? ViewState.shrink : ViewState.enlarge)
                  : (flightDirection == HeroFlightDirection.push ? ViewState.enlarge : ViewState.shrink),
...
class HeroTextWidget extends StatelessWidget {
  ...
  const HeroTextWidget({required this.tag, super.key, this.child});
