...
class _AnimatedButtonState extends State<AnimatedButton> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    ).animate(CurvedAnimation(parent: widget.controller!, curve: const Threshold(.65)));

    _ringThicknessAnimation = Tween<double>(
      begin: _loadingCircleRadius,
      end: _loadingCircleThickness,
    ).animate(CurvedAnimation(parent: widget.controller!, curve: const Interval(.65, .85)));
    _ringOpacityAnimation = Tween<double>(
      begin: 1,
      end: 0,
    ).animate(CurvedAnimation(parent: widget.controller!, curve: const Interval(.85, 1)));
  ...
  void _updateColorAnimation() {
    ...
    ).animate(CurvedAnimation(parent: widget.controller!, curve: const Interval(0, .65, curve: Curves.fastOutSlowIn)));
  }
  ...
  Widget _buildButtonText(ThemeData theme) {
    return FadeTransition(
      ...
      child: AnimatedText(text: widget.text, style: theme.textTheme.labelLarge),
    );
  }
      ...
      children: <Widget>[
        FadeTransition(
          ...
          child: AnimatedBuilder(
            ...
            builder: (context, child) => Ring(color: widget.loadingColor, thickness: _ringThicknessAnimation.value),
          ),
        ),
        if (_isLoading)
          SizedBox(
            ...
            child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color?>(widget.loadingColor)),
          ),
        _buildButton(theme),
      ],
    );
